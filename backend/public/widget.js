;(function () {
  ...
  function injectStyles(c) {
    ...
    s.id = 'pa-css'
    s.textContent = `
      #pa-root * { box-sizing:border-box; font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif; margin:0; padding:0; }
      #pa-btn {
        position:fixed; bottom:24px; right:24px; z-index:2147483640;
        width:56px; height:56px; border-radius:50%; border:none;
        background:${c}; cursor:pointer;
        display:flex; align-items:center; justify-content:center;
        box-shadow:0 4px 20px rgba(0,0,0,.22); transition:transform .2s;
      }
      #pa-btn:hover { transform:scale(1.08); }
      ...
      .pa-x { margin-left:auto; background:none; border:none; cursor:pointer; color:#fff; opacity:.75; padding:4px; display:flex; }
      ...
      .pa-bubble { padding:9px 13px; border-radius:16px; font-size:13.5px; line-height:1.55; word-break:break-word; }
      .pa-row.b .pa-bubble { background:#fff; color:#1e293b; border-bottom-left-radius:4px; box-shadow:0 1px 3px rgba(0,0,0,.07); }
      .pa-row.u .pa-bubble { background:${c}; color:#fff; border-bottom-right-radius:4px; }
      ...
      #pa-lf { padding:18px 16px; display:flex; flex-direction:column; gap:10px; background:#fff; }
      ...
      #pa-skip { text-align:center; font-size:12px; color:#94a3b8; cursor:pointer; }
  ...
  function open() {
    ...
    if (chatHistory.length === 0) {
      appendBot(config.welcomeMessage || 'Hi! How can I help you today?')
    } else {
  ...
  function buildLeadForm() {
    return `
      <div id="pa-lf">
        ...
        <p>Share your details to continue chatting.</p>
        <input id="pa-fn" class="pa-fi" type="text"  placeholder="Your name *"/>
        <input id="pa-fp" class="pa-fi" type="tel"   placeholder="Phone number${config.requirePhone ? ' *' : ' (optional)'}"/>
  ...
  function buildInputForm() {
    return `
      <div id="pa-form">
        ...
        <button id="pa-send">${iconSend()}</button>
      </div>`
  }
  ...
  async function submitLead() {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/api/leads/capture`, {
        ...
        body: JSON.stringify({ botId: BOT_ID, name, phone, email, firstMessage: firstMsg, sourcePage: window.location.href, sessionId }),
      })
      const data = await res.json()
      if (data.leadId) { leadId = data.leadId; localStorage.setItem(LEAD_KEY, leadId) }
    } catch (_) {}
    leadCaptured = true
    activateChat()
  }

  function skipLead() { leadCaptured = true; activateChat() }
  ...
  async function sendMsg() {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/chat`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ botId: BOT_ID, sessionId, message: text, history: chatHistory.slice(-8), leadId: leadId || undefined }),
      })
      const data = await res.json()
      hideTyping()
      appendBot(data.answer || data.response || data.message || "Sorry, I couldn't get a response.")
    } catch {
      hideTyping()
  ...
  function appendBot(text) { appendBotUI(text); chatHistory.push({ role: 'assistant', content: text }) }
  ...
  function iconChat() {
    return `<svg viewBox="0 0 24 24" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg>`
  }
  function iconX() {
  ...
  function iconSend() {
    return `<svg viewBox="0 0 24 24" fill="currentColor"><path d="M22 2L11 13M22 2l-7 20-4-9-9-4 20-7z"/></svg>`
  }
